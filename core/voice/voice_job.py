"""Execution job structure for voice cloning speech synthesis tasks.
"""
...
class VoiceJob:
    """Tracks state and progress parameters for a background speech synthesis task."""

    def __init__(self, config: VoiceConfig, job_id: Optional[str] = None) -> None:
        """Initialize VoiceJob.
        ...
        self.config = config
        self.status = "pending"  # pending, downloading_weights, running, completed, failed
        self.progress = 0.0
    ...
    def update_status(self, status: str, progress: float, error_message: Optional[str] = None) -> None:
        ...
        if status in ["completed", "failed"]:
            ...
            if status == "completed":
                self.output_path = self.config.output_audio_path
